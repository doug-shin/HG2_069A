...
#### 설명
이 프로그램은 CAN 통신으로 오는 프로토콜 규격에 따른 메시지를 받아서 모듈을 제어하고 보고하게 하는 프로그램이야. CAN통신에는 빅엔디안을 사용하고 모듈은 리틀엔디안을 사용하므로 엔디안 변환이 필요해.
CAN통신과 모듈에서 사용 단위도 다른데, CAN에서는 전류 단위는 ma, 전압은 mv, 시간단위는 10ms, 온도단위는 0.1도C야. 모듈에서는 전류 A, 전압 V, 시간 ms, 온도 1도C(float)야.
...
작동 중에는 100번대로 10ms 사이클로 10ms 마다 8개의 메시지를 보고하고 작동 중이 아닐 때는 110번대로 100ms마다 8개 메시지를 보고하는 거야.
100번대 메시지와 110번대 메시지는 id와 보고 주기만 다르고 구조는 같아.
제어명령은 200번대 메시지가 들어오는데 200~209로 동작 조건이 주어진 뒤 210 제어명령에서 시작하라고 하면 동작에 들어가고 100번대 보고하면 돼. 250번대 글로벌 조건은 주어질 수도 있고 없을 수도 있는데 주어질 경우 처음 200번대 메시지 들어오기 전에 주어질꺼야. 글로벌 조건은 모든 스텝에 다 적용하면 돼. EPC는 사용자가 설정한 여러개의 스텝을 스텝 단계마다 스텝명령으로 보낼건데 전체가 몇 스텝인지는 모듈에는 가르쳐주지 않아.
안전조건 미충족시 일시정지하고 종료조건 충족시 정지해. 혹은 210 제어명령에서 일시정지, 정지 명령이 와도 작동을 종료하는거야. 이렇게 작동이 정지, 일시정지 등을 하게 될 때는 130번대로 종료 보고를 해야해. 그 뒤에는 110번대로 100ms 보고를 하는거야.
